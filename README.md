...
N.G. March, E. J. Carr, and I. W. Turner (2021). A fast algorithm for semi- analytically solving the homogenization boundary value problem for block locally- isotropic heterogeneous media. Applied Mathematical Modelling 92, 23–43.
...
Now that the image has been imported and converted to an array of diffusivity values, we can choose a pixelation parameter, r, that determines the size of the geometry that will be homogenized.
...
Any element of the array ``factors`` is an acceptable value for ``k`` or ``r``, so we can loop through a subset of these values to generate different pixelated geometries:
